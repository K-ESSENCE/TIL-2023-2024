...
반이중
대화를 관리하기위해서 토큰이 사용됨.
송수신측간의 대화동기를위해 전송하는 정보의 일정부분에 체크점을 두어서 수신상태를 체크함
이 체크점을 동기점이라고함.
동기점은 오류가 있는 데이터의 회복을 위해 사용하는것. 소동기점과 대동기점이있음.
다운로드할때 이어받기하시겠습니까? 가 가능한이유가 동기점때문임.

- 소동기점
  하나의 대화 단위안에서 데이터의 전달을 제어하는 역할 수신측으로부터 확인 ack를 받지않음

- 대동기점
  전송하는 각 데이터의 처음과 끝에 사용하여 전송하는 데이터 단위를 대화단위로 구성하는역할
  얘는 ack를 반드시받음.

### 표현 계층

서로 다른 데이터 표현 형태를 갖는 시스템간의 상호접속을 위해 필요한 계층.
코드변환/데이터 암호화/ 데이터 압축/ 구문 검색/ 정보 형식 변환 /문맥관리 기능을 함.

데이터 암호화형태를 뭘로했는지. 알려줘야 상대방이 볼것

### 응용 계층

사용자가 OSI환경에 접근할수있도록 제공해주는것을 말함.
정보 교환/ 전자 사서함/ 파일전송/ 가상터미널 등을 제공함

### 경로 제어 프로토콜

routing protocol
효율적인 경로 제어를 위해 네트워크 정보를 생성,교환,제어 하는 프로토콜을 총칭함.
대표적인 경로 제어 프로토콜에는 IGP, EGP ,BGP가 있음.

외부 내부 프로토콜 => I/E

자율시스템간에 라우팅을 해야됨.

출발지에서 목적지까지 찾아갈때 찾아야되는 네트워크 갯수를 "홉"이라고 부름.

그래서 이 홉이 짧은 경로를 선택해주는게 RIP (?)

- RIP
  현재 가장 널리 사용되는 라우팅 프로토콜로 거리 벡터 라우팅 프로토콜이라고도 불림, 최단경로 탐색에 Bellman-Ford/벨만 포드 알고리즘이 사용됨.
  소규모 동종의 네트워크(자율시스템,AS)내에서 효율적인 방법.
  최대 홉수를 15개로 제한 (거쳐가는애들을 15개로 제한) 15개 네트워크를 초과하는 경우 도달불가능한 네트워크 임. => 대규모 네트워크에서는 RIP사용불가.
  라우팅 정보를 30초마다 네트워크 내의 모든 라우터에 알리며, 180초 이내에 새로운 라우팅 정보가 수신되지않으면 해당 경로를 이상 상태로 간주함.
  180초 이내에 새로운 라우팅 정보가 수신되지 않으면. 해당 경로를 이상 상태로 간주함.

- OSPF
  다익스트라 알고리즘 사용
  RIP의 단점 해결 => 대규모 네트워크에 많이 사용됨.
  라우팅 정보에 변화가 생길 경우 변화된 정보만 네트워크 내의 모든 라우터에 알림.
  하나의 자율 시스템에서 동작하면서 내부 라우팅 프로토콜의 그룹에 도달.

  벨만포드와 다익스트라의 큰 차이점은 음수값을 처리할수있냐 없냐인데. 벨만포드는 음수값을 처리가능하고 다익스트라는 처리 불가.

  라우터들의 집합이 as (자율시스템) 이고 자율시스템이고
  망에서 봤을때 수없이 많이 자율시스템이 있을텐데
  자율시스템간의 라우팅을 해줘야되는데 이게 외부 게이트웨이 프로토콜

  외부 게이트웨이 프로토콜

  -EGP
  자율 시스템 간의 라우팅 / 게이트웨이 간의 라우팅

  -BGP
  자율 시스템간의 라우팅/ EGP의 단점을 보완하기 위해서 만들어졌음.
  초기에 BGP라우터들이 연결될때에는 전체 경로 제어표(라우팅 테이블)를 교환하고 이후에는 변화된 정보만 교환함.

### 회선 교환 방식.

통신을 원하는 . 두지점을 교환기를 이용하여 물리적으로 접속시키는 방식, 기존의 음성 전화망이 대표적.
데이터 전송전에 먼저 통신망을 통한 연결이 필요.

회선 교환망이라는건 데이터 전송전에 통신망을 통한 연결이필요 => 상대방이 전화안쓰고있을 때 우리집 전화기로 전화걸면 걸자말자말하는게아니라. 상대가받아야 되는것처럼. 통신망을 통한 연결이 필요함.

한번 접속하고 나면 . 그 통신회선은 전용회선에 의한 통신처럼 데이터가 전달됨(고정 대역 전송.)

통신과정 : 호(링크)설정 -> 데이터 전송 -> 호(링크)해제
한번 접속이 이루어지게되면 별도로 해제하기전까지는 마치 전용선으로 연결된것처럼 실시간으로 데이터 전송이 가능한것

### 패킷 교환 방식

패킷교환방식=> 메시지를 일정한 길이의 패킷으로 잘라서 전송하는 방식. / 응답시간이 빠르므로 대화형 응용이 가능
음성보단 데이터 전송에 더 적합.

크드 및 속도 변환이 가능.

패킷망 상호 간의 접속을위한 프로토콜은 X.75임.

하나의 회선을 여러 사용자가 공유할 수 있으므로 회선 이용률이 높다.

통신량의 제어를 통한

전송혹은 다중화를 목적으로 메시지를 일정한 비트수로 분할하여 송 수신측 주소와 제어정보를 부가하여 만든 데이터 블록.

PAD: 패킷을 조립/ 분해해주는것

패킷 교환망의 기능: 패킷 다중화, 경로제어, 논리채널, 순서제어 ,트래픽제어, 오류제어

패킷 교환방식은 두가지가있음

가상회선 방식과 데이터 그램 방식

가상회선 방식은 자전거 동호회와같음. 다같이 어디로 가겠다고 약속하면 전부다 장비를 갖추고 자전거를 끌고 모이는 집합장소로 모임. 거기까지 자전거 전용도로로 갈거라고 하면 가장 안전하고 빠르게 찾아갈수있는 길을 결정. 연결 확립후 순차적으로 운반됨

데이터 그램 방식은 연결 경로 설정 안하고 순서 상관없이 독립적으로 운반하는 방식.

### LAN

근거리 통신망 => 짧은 거리를 보고 말함 학교/회사/연구소등 한 건물이나 일정 지역 내에 컴퓨터나 단말장치들을 고속전송 회선으로 연결해서 서로 공유할수있도록 한 네트워크 형태.

WAN 은 광대역 통신망
근거리 통신망은 경로 설정을 할 필요가없음.

오류가 발생하는것도 광대역 통신망일때 더 많은 오류가 발생.
근거리 통신망은 거리가 짧기때문에 광대역 전송 매체를 사용해서 고속 통신이 가능.

전송 매체로 꼬임선 동축 케이블 광섬유 케이블

전송 방식으로는 베이스밴드와 브로드밴드 방식이있음 => 베이스밴드는 변조를 안하는 방식이라서 따로 모뎀이 필요없음
브로드 밴드 방식은 신호를 변조를 해서 보냄

망의 구성 형태에따라 스타형,버스형,링형,트리형,망형으로 분류 가능

LAN의 게층 구조는 물리계층과 데이터 링크 계층으로 나누어짐.

물리계층은 osi 7계층의 물리계층과 동일

데이터링크 계층에서는 매체 접근 제어 계층과 링크 제어 계층으로 나누어짐.
