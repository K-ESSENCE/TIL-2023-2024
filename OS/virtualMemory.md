...
레퍼런스 0이라는 이야기는 시계 가 한바퀴 돌아오는 동안 참조가 없었다는 의미

1이라는 의미는 적어도 1번의 참조가 있었다는 이야기

하드웨어가 이 페이지들을 참조할때 레퍼런스빗을 1로만들어버리는 역할을 하고

운영체제는 하드웨가 만든것을 보고 1인건 0으로 만들고 0인건 쫓아냄.

이게 클락 알고리즘

즉 LRU처럼 가장오래된걸 쫓가내는건 아님

하드웨어는 bit하나를 세팅해줌 refernce bit

reference bit말고도 modified bit이라는것을 하나 더 들고있기도함.

dirty bit이라고도하는데

어떤 페이지가 쫓겨날때. 이페이지의 modified bit이 0이라고하면 백킹스토어에서 물리적인 메모리에 올라온뒤로write가 발생하지않은것임.
그래서 이런애들을 쫓아낼떄는 메모리에서 아예 쫓겨내면됨

반면에 1이라면 메모리에 올라온 이후로 적어도 한번은 cpu에서 write를 한것이기때문에
...
동시에 메모리에 올라가는 프로그램 갯수를 조절해야됨.

그렇게 함으로써 프로그램이 어느정도는 메모리 확보할 수 있도록해줘야됨
이걸해주는게 PFF랑 Working-Set Model

적어도 프로그램들이 원활하게 실행될려면 어느정도의 메모리페이지를 가지고있어야될까 하는것.

프로그램이 일정시간 ,특정시간에는 특정 메모리위치만 집중적으로 참조하는것을 Locality of reference라고 부름

집중적으로 참조되는 해당 page들의 집합을 locality set이라고함.

locality에 기반하여 프로세스가 일정 시간 동안 월활하게 수행되기 위해 한꺼번에 메모리에 올라와 있어야 하는 page들의 집합을 working set 이라 정의함.

multiprogramming degree를 조정해서 thrashing을 방지함.
...
프로그램한테 5개 필요하다하면 올려놓고

5개필요한데 2개밖에못올린다하면 2개를 올리는게아니라 swap

현재부터 과거 윈도우크기 델타 이내에 들어오는 페이지들은 워킹셋으로 간주해서 그 페이지들은 메모리에 유지.

참조된후 유지하다가 버림.

버릴려고했는데 다시 참조되면 또 유지하고.
...
특정 프로그램이 페이지 폴트를 얼마나 내는지 봄

페이지폴트를 많이일으키는애한테 더많은 페이지를 줌.

반대로 어떤 프로그램은 너무 발생을 안함.

그럼 페이지를 너무많이 가지고있다고 생각하면 메모리를 또 빼앗아감.

여기서도 더 줄메모리가없으면
그프로그램을 통쨰로 swap out 시킴

Global replacement
미리 할당을 하지않더라도. 알고리즘이 알아서 할당하는것과 같은 효과를 내는 알고리즘들이 위의 2개

### Page Size의 결정

![alt text](image-88.png)

페이지 사이즈가 작으면 페이지 테이블 이 커져야

Disk transfer의 효율성이 페이지 크기가 클수록 증가.

디스크 헤드가 이동해서 특정위치에가서 섹터를 읽거나 써야되는데. 이 Seek하는 시간이 대단히 오래걸림

그래서 가능하면 한번이동해서 많은양을 읽어올리는게 좋기때문에 페이지가 큰게 좋은것

너무 작으면 그때그때 페이지폴트가 나고 디스크가 이동하는 Seek시간이 계속발생하면 좋지않다는 것
