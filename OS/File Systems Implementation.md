...
파일의 데이터를 읽고쓰는 방법은 두가지 인터페이스가있는데

하나는 read write 시스템콜을 하는것

또다른 하나는 memory-mapped I/O

파일에 매핑하는것. 그래도 디스크에있는 파일을 읽어오는건 똑같다.

그리고 그 내용을 page cache에 카피

매핑을 해놨으면 내메모리에다가읽고쓰는.

map만해놓고 메모리에 안읽어왔다면 그러면 접근하려고할때 페이지 폴트가일어남

그러면 운영체제로 가서 다시 내용을 읽어오는.

그래서 두 방법이 진행하는 경로는 똑같은데

차이점은 read/write 시스템콜을 쓸떄는 버퍼 캐시에 있던 말던 운영체제한테 요청해서 받아오는것이고

memory mapped를 사용하면 page cache에 올라온내용은 운영체제의 도움없이 자기 메모리 영역을 접근하는 방식을 통해서 i/o를 한다.

커널없이 사용하는게 memory mapped를 사용하는이유

근데 어떤 걸 쓰던 buffer cache를 쓰긴써야됨. 그래서 양쪽 모두 버퍼 캐시에있는 내용을 자기의 페이지 캐시에 복제하는 오버해드가있다.

최근의 Unified buffer cache는
기존의 buffer cache처럼 운영체제가 공간을 따로 나누어놓지않고 필요에따라 page cache에서 공간을할당해서 사용함.

그럴경우에는 경로가 더더욱 단순해진다.

### 프로그램의 실행

프로그램이 파일시스템의 실행파일 형태로 저장되어있다가 실행파일을 실행시키면 프로세스가 됨

프로세스가 되면 프로세스만의 독자적인 주소공간 = 버츄얼 메모리가 만들어지고

코드 데이터 스택이라는것으로 이루어진 주소공간이 생긴다.

프로그램이 실행되다가 데이터파일을 쓰고싶은데 mmap()형태로 쓰고싶다고 하면 이 파일의 일부를 내 주소공간에 매핑해달라고 시스템콜을 하는것.

그래서 이데이터파일의 일부를 프로그램의 버츄얼 메모리 주소공간에 매핑해둔다.

만약 그내용이 안올라와있다 하면 페이지폴트가일어나고 cpu가 os로 넘어가서 물리적인 메모리로 올려놓을것임

그럼 가상 페이지가 물리적인 페이지로 매핑이되는과정.

그럼 이때부터는 자신의 os의 도움을 받지않고 자기 주소공간의 일부라서 읽거나쓰거나가능

나중에 내용이 메모리에서 쫓겨날때는 스왑에어리어로 가는게 아니라 메모리맵드 기때문에 파일에다가 쓰고 쫓아내야됨.

또다른 프로그램 도 마찬가지로 mmap을 호출할수있음. 그경우에는 내용이 쉐어가됨.

이프로세스 저프로세스 에 물리적인공간에 매핑되는.

똑같은 파일에대해서 리드라이트 시스템콜을 사용할수도.
요청했을때 버퍼캐시에 이미 올라와있으면 내용 카피해서 사용자프로세스한테 전달을 해준다.

사용자 프로그램이 파일을 접근하는건 어쨌든 리드라이트 시스템콜이랑 메모리 맵드 두가지가 있다.
