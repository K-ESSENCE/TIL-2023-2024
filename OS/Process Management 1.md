...
참고로 스케쥴러와 디스페쳐 둘다 하드웨어가 아니고 운영체제에 있는 특정 기능을 이렇게 부른다.
![Alt text](image-14.png)

CPU 누구한테 줄지 => 스케쥴러

디스패쳐는 실제로 CPU를 주는 과정

CPU스케쥴링이 언제 필요한가?

1.Running => Blocked (I/O에서 요청하는 시스템 콜)

CPU를 어떤 프로세스가 잡고있다가 오래걸리는 작업을하러 간 경우 (자진해서 CPU를 내어놓는경우)

2. 계속 쓰고 싶은데 무한정 줄수없어서 빼았는것 timer interrupt

3. I/O를 요청했던 프로세스 를 다시 ready

4. Terminate

1,4 에서의 스케쥴링은 자진반납을 함
